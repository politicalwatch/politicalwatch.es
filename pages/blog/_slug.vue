  ...
  <section class="o-container c-post o-section">
    ...
    <div class="c-post__meta">
      ...
      <div class="c-post__share">
        {{ $t("global.shareLabel") }}
        <a
          :href="`https://twitter.com/intent/tweet?url=${$config.baseURL}${post.path}`"
          target="_blank"
        >
          <twitter />
        </a>
        <a
          :href="`https://api.whatsapp.com/send?text=${$config.baseURL}${post.path}`"
          target="_blank"
        >
          <whatsapp />
        </a>
        <a
          :href="`https://www.facebook.com/sharer/sharer.php?u=${$config.baseURL}${post.path}`"
          target="_blank"
        >
          <facebook />
        </a>
        <a
          :href="`https://t.me/share/url?url=${$config.baseURL}${post.path}`"
          target="_blank"
        >
          <telegram />
        </a>
        <a
          :href="`https://www.linkedin.com/shareArticle?mini=true&url=${$config.baseURL}${post.path}`"
          target="_blank"
        >
          <linkedin />
        </a>
...
export default {
  ...
  async asyncData({ $content, app, params }) {
    ...
    const author = await $content(
      `${app.i18n.locale}/equipo`,
      post.author
    ).fetch();
    const related = await $content(`${app.i18n.locale}/blog`)
      .where({ slug: { $in: post.related } })
      .fetch();
