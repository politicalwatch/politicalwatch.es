  ...
  <div class="c-search">
    ...
    <div class="c-search__form">
      <input
        ...
        type="search"
        :placeholder="$t('blocks.search.placeholder')"
        @change="search"
      />
      ...
      <div v-if="searched == true" class="c-search__result">
        ...
        <ul v-if="qhdlResults.length > 0">
          <li v-for="item in qhdlResults" :key="item.url">
            <a target="_blank" :title="item.full" :href="item.url">{{
              item.title
            }}</a>
            <span v-for="tag in item.tags" :key="tag" class="c-search__tag">{{
              tag
            }}</span>
          </li>
        </ul>
        <a
          v-if="qhdlResults.length > 0"
          target="_blank"
          :href="qhldMore"
          class="c-link"
          >{{ $t("blocks.search.see_more") }}</a
        >
        <a
          v-if="qhdlResults.length == 0"
          target="_blank"
          href="https://quehacenlosdiputados.es/buscar"
          class="c-link"
          >{{ $t("blocks.search.activity_link") }}</a
        >
      </div>
      <div v-if="searched == true" class="c-search__result">
        ...
        <a
          v-if="ompResults.length == 0"
          target="_blank"
          href="https://buscador.openmanifestoproject.org"
          class="c-link"
          >{{ $t("blocks.search.proposals_link") }}</a
        >
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    qhldSearch(term) {
      ...
      const url =
        "https://api.quehacenlosdiputados.es/initiatives/?page=1&text=" + term;
    ...
    ompSearch(term) {
      ...
      const url =
        "https://api.openmanifestoproject.org/proposals?threshold=1&page=1&tags=" +
        searchTerm;
      this.ompMore =
        "https://buscador.openmanifestoproject.org/results/threshold=1&tags=" +
        searchTerm;
        ...
        .then((data) => {
          ...
          while (count < 5) {
            const result = {
              ...
              url:
                "https://buscador.openmanifestoproject.org/proposal/" +
                data[count].id,
